...
Deno.serve(async (req) => {
  ...
  try {
    ...
    return new Response(JSON.stringify({ battle_id: battleId.id, winner: result.winner, player, rival, events: result.events }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
  } catch (error) {
